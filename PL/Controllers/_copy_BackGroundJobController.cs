...
{
    public class _copy_BackGroundJobController : Controller
    {
        #region props
        public _copy_ApisController apis = new _copy_ApisController();
        ...
        public ActionResult BackgroundJobCreateReport(ML.Historico aHistorico)
        {
            try
            {
                ...
                if (boolExisteReporte(aHistorico, AnioActual) == true)//Eliminar el registro
                {
                ...
                {
                    /*
                     * Se debe generar el reporte a nivel GAFM
                     * El de la Uniad de negocio actual se saca de todos estos calculos
                     * El de las otras unidades de negocio se sacaria de la tabla de historicos tomando el mismo año que se esta consultando en AnioActual
                    */
                }

                //5 mejores EE y EA (9, 10)
                ...
                /*Data de la primer pantalla (6)*/
                objProcesosOrg = apis.getProcesosOrga(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objPracticasCulturales = apis.getPracticasCulturales(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objEsperadas = apis.getEncuestasEsperadas(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objParticipacion = apis.getParticipacion(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objCalificacionGlobal = apis.getCalificacionGlobal(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objConfianza = apis.getConfianza(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objNivelCompromiso = apis.getNivelCompromiso(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objNivelColaboracion = apis.getNivelColaboracion(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                /*Data de la segunda pantalla (7)*/
                objCredibilidad = apis.getCredibilidad(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objImparcialidad = apis.getImparcialidad(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objOrgullo = apis.getOrgullo(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objRespeto = apis.getRespeto(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objCompanierismo = apis.getCompanierismo(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                /*Data de la tercer pantalla (8)*/
                objCoaching = apis.getCoaching(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objHabgerenciales = apis.getHabGerenciales(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                objAlineacionEstrategica = apis.getAlineacionEstrategica(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                
                objCambio = apis.getCambio(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                //Bienestar (17)
                var aFiltrosEntUnNivelAbajo = getFiltros(idUneg, aHistorico.IdTipoEntidad, aHistorico.EntidadId, aHistorico.EntidadNombre);
                objBienestarEE = apis.getPorcentajePsicoSocialEE(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg, aFiltrosEntUnNivelAbajo);
                objBienestarEA = apis.getPorcentajePsicoSocialEA(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg, aFiltrosEntUnNivelAbajo);
                //
                ...
                objNube1 = apis.getDatosNube(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, "1");
                objNube2 = apis.getDatosNube(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, "2");
                objNube3 = apis.getDatosNube(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, "3");
                objNube4 = apis.getDatosNube(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, "4");
        ...
        public static List<_copy_ApisController.myCustomArray> getHijosEstructura(int unidadNegocio, int? criterioSeleccionado, int? entidadId)
        {
            int IentidadId = Convert.ToInt32(entidadId);
            //Trae el nivel inmediato inferior para pintar el grid de bienestar
            BackGroundJobController s = new BackGroundJobController();
            ...
            var finalColumnas = new List<_copy_ApisController.myCustomArray>();
            //data.Data
            foreach (ML.Company item in estructura)
            {
                if (!String.IsNullOrEmpty(item.CompanyName))
                {
                    finalColumnas.Add(new _copy_ApisController.myCustomArray { type = "Comp=>", value = item.CompanyName });
                }
                if (!String.IsNullOrEmpty(item.Area.Nombre))
                {
                    finalColumnas.Add(new _copy_ApisController.myCustomArray { type = "Area=>", value = item.Area.Nombre });
                }
                if (!String.IsNullOrEmpty(item.Area.Departamento.Nombre))
                {
                    finalColumnas.Add(new _copy_ApisController.myCustomArray { type = "Dpto=>", value = item.Area.Departamento.Nombre });
                }
                if (!String.IsNullOrEmpty(item.Area.Departamento.Subdepartamento.Nombre))
                {
                    finalColumnas.Add(new _copy_ApisController.myCustomArray { type = "SubD=>", value = item.Area.Departamento.Subdepartamento.Nombre });
                }
            }
        ...
        public ActionResult getByIndicador([FromUri]int id, [FromBody]ML.Historico aHistorico)
        {
            ...
            {
                case 1:
                    data = apis.getParticipacion(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                    break;
                case 2:
                    data = apis.getCalificacionGlobal(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                    break;
                case 3:
                    data = apis.getConfianza(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                    break;
                case 4:
                    data = apis.getNivelCompromiso(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                    break;
                case 5:
                    data = apis.getNivelColaboracion(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                    break;
                case 6:
                    data = apis.getCredibilidad(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                    break;
                case 7:
                    data = apis.getImparcialidad(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                    break;
                case 8:
                    data = apis.getOrgullo(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                    break;
                case 9:
                    data = apis.getRespeto(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                    break;
                case 10:
                    data = apis.getCompanierismo(criterioBusquedaSeleccionado, aHistorico.EntidadNombre, unidadNeg);
                    break;
                default:
                    break;
            }
        ...
        public ActionResult getByIndicador_3([FromUri]int id, [FromBody]List<ML.minHistorico> aHistorico)
        {
            ...
            foreach (var item in aHistorico)
            {
                ...
                {
                    case 1:
                        data = apis.getParticipacion(criterioBusquedaSeleccionado, item.EntidadNombre, unidadNeg);
                        result.Add(data.Data);
                        break;
                    case 2:
                        data = apis.getCalificacionGlobal(criterioBusquedaSeleccionado, item.EntidadNombre, unidadNeg);
                        result.Add(data.Data);
                        break;
                    case 3:
                        data = apis.getConfianza(criterioBusquedaSeleccionado, item.EntidadNombre, unidadNeg);
                        result.Add(data.Data);
                        break;
                    case 4:
                        data = apis.getNivelCompromiso(criterioBusquedaSeleccionado, item.EntidadNombre, unidadNeg);
                        result.Add(data.Data);
                        break;
                    case 5:
                        data = apis.getNivelColaboracion(criterioBusquedaSeleccionado, item.EntidadNombre, unidadNeg);
                        result.Add(data.Data);
                        break;
                    case 6:
                        data = apis.getCredibilidad(criterioBusquedaSeleccionado, item.EntidadNombre, unidadNeg);
                        result.Add(data.Data);
                        break;
                    case 7:
                        data = apis.getImparcialidad(criterioBusquedaSeleccionado, item.EntidadNombre, unidadNeg);
                        result.Add(data.Data);
                        break;
                    case 8:
                        data = apis.getOrgullo(criterioBusquedaSeleccionado, item.EntidadNombre, unidadNeg);
                        result.Add(data.Data);
                        break;
                    case 9:
                        data = apis.getRespeto(criterioBusquedaSeleccionado, item.EntidadNombre, unidadNeg);
                        result.Add(data.Data);
                        break;
                    case 10:
                        data = apis.getCompanierismo(criterioBusquedaSeleccionado, item.EntidadNombre, unidadNeg);
                        result.Add(data.Data);
                        break;
                    default:
                        break;
        ...
        public ActionResult getByReactivo([FromUri]int id, [FromBody]List<ML.minHistorico> aHistorico)
        {
            ...
            foreach (var item in aHistorico)
            {
                ...
                var data1 = repo.GetPorcentajeAfirmativasEnfoqueEmpresa(model);
                //EA
                model.IdPregunta = getIdPreguntaEA(model.IdPregunta);
                var data2 = repo.GetPorcentajeAfirmativasEnfoqueEmpresa(model);
